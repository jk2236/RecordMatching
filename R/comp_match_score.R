...
#' insert description.
#' 
#' @param cott.vec A vector of Cotterman identity coefficients.
#' @param marker The name of a STR locus under consideration.
#' @param str.f A file containing *known (NOT imputed)* STR genotypes. 
#' @param data.dir A name of a directory (including full path) containing imputed STR info
#'                 and the STR allele frequencies from the reference panel. If `NULL` (default),
#'                 the system default `save.dir` is used.  
#'
#' @return A match score matrix based on a single STR locus. 
#' @export
match.mat.single <- function(cott.vec, marker, str.f, data.dir=NULL) {
    
    del.vec <- cott.to.jacq(cott.vec)
    
    data.dir <- paste(base.dir, 'beagle_data/', sep='')
    save.dir <- paste(base.dir, 'result/', sep='')
    if(!dir.exists(save.dir)) {
        dir.create(save.dir)
    }
    
    bgl.dir.A <- data.dir #  STR
    bgl.dir.B <- data.dir #  SNP
    ...
    for (m in 1:n.marks) {
        ...
        marker.name <- as.character(marker.info[m, 1])
        # print(paste('marker', counter, 'out of', n.loci, ':', marker.name))
        
        # Allele frequency from training set of A
        al.freq.filename <- paste(bgl.dir.A, 
                                  "train_allelefreqs/train_freqs_", 
                                  marker.name, ".frq", sep = "")
        
        # STR genotype probability from imputation (BEAGLE) of B
        gp.filename <- paste(bgl.dir.B, 
                             "imputed_val_STRonly/valimp_STR_", 
                             marker.name, ".GP.FORMAT", sep = "")
        
        # Matching genotype (known) for A
        gt.filename <- paste(bgl.dir.A, 
                             "unphased_val_STR/val_STR_", 
                             marker.name, ".GT.FORMAT", sep = "")
        
        ## TRUE genotypes of A, i.e. known STR genotype, NOT from imputation.
        ## The missing STR genotype is represented with NA
        val.gen.str.true <- read.gt(gt.filename, phased=F)
        ...
        # imagesc(lrmat, xlab='B', ylab='A')
        
        llr.single.irt[[m]] <- lrmat
        names(llr.single.irt)[m] <- marker.name
        # tmp <- tmp + lrmat
    }
    
    ## Compute match score under three schemes
    #  Hungarian accuracy computation
    ...
    ## Save llr matrix
    if(!dir.exists(paste(save.dir, paste(del.vec, collapse="_"), sep=''))) {
        stop('The save folder does not exist')
    }
    
    setwd(paste(save.dir, paste(del.vec, collapse="_"), sep=''))
    save.name <- 'llr.RData'
    save(llr.single.irt, file=save.name)
    # assign(save.name, llr.single.irt)
    setwd(cur.dir)
